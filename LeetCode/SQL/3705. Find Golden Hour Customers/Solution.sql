...
with peak_hours_cte AS (
    SELECT r.customer_id
            , r.order_id
            , CASE 
                WHEN TIME(r.order_timestamp) BETWEEN '11:00:00' AND '14:00:00' THEN 1
                WHEN TIME(r.order_timestamp) BETWEEN '18:00:00' AND '21:00:00' THEN 1
                ELSE 0
            END AS peak_hour
            , r.order_rating
            , CASE 
                WHEN order_rating IS NULL THEN 0
                ELSE 1
            END AS null_rating
...
, calc_cte AS (
    SELECT customer_id
            ...
            , SUM(peak_hour) AS peak_hour_orders
            , ROUND(((CAST(SUM(peak_hour) AS FLOAT)*100.0)/CAST(COUNT(DISTINCT order_id) AS FLOAT)), 0) AS peak_hour_percentage
            , ROUND(AVG(order_rating), 2) AS average_rating
            , SUM(null_rating) AS null_rating_orders
    FROM peak_hours_cte
    GROUP BY 1
    HAVING COUNT(DISTINCT order_id) >= 3
    AND ((CAST(SUM(peak_hour) AS FLOAT)*100.0)/CAST(COUNT(DISTINCT order_id) AS FLOAT)) >= 60
    AND ROUND(AVG(order_rating), 2) >= 4.0
    AND (CAST(SUM(null_rating) AS FLOAT)*100.0)/CAST(COUNT(DISTINCT order_id) AS FLOAT) >= 50
)
SELECT customer_id
        , total_orders
